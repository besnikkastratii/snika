...
if ($user['pubgid'] === $pubgidd) {
   ...
   array_push($errors, "username  is too short,minimum 5 characters");
}

		if (strlen($password_1) < 6){
   array_push($errors, "password is too short,minimum 6 characters");
}
  }

$end_date  = date("Y/m/d H:i:s", strtotime($date . ' + 10 days'));
...
$query = "SELECT * FROM users WHERE username='$username' or email='$username' or pubgid='$username' AND password='$password'";
$result=mysqli_query($db,$query);
$num=mysqli_num_rows($result);
while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) 
if($row['expiryD'] < $date)
	  {
 array_push($errors, "Your account has Expired");
}	
